# -*- coding: utf-8 -*-
# listener.py (v2) — Escucha TODOS los canales/supergrupos y publica en Redis Streams
...
phone        = os.getenv("TELEGRAM_PHONE", "+34607190588")  # <-- pon tu número si no usas .env
...
# TTL para claves de dedup (evita re-procesar el mismo canal|msg|rev). 15 días por defecto.
DEDUP_TTL_SEC = int(os.getenv("DEDUP_TTL", str(15*24*3600)))
# Límite aproximado del stream (para retención)
...
async def publish_to_stream(r: Redis, payload: dict):
    """Publica JSON en el stream de parseo."""
    data = json.dumps(payload, ensure_ascii=False)
    await r.xadd(PARSE_STREAM, {"data": data}, maxlen=STREAM_MAXLEN, approximate=True)
...
async def main():
    ...
    log(f"Publicando en Redis Stream: {PARSE_STREAM}. CSV={'ON' if WRITE_CSV else 'OFF'}. Ctrl+C para salir.")
    ...
    async def on_new(event: events.NewMessage.Event):
        ...
        # Dedup (si ya se publicó exactamente esta revision, no repetir)
        ...
        await publish_to_stream(r, payload)
