...
STOP_WORD  = r"(?:\bstop\b)"
ENTRY_HINTS = r"(?:\bentry\b|\bentrada\b|\bbuy\s*at\b|\bsell\s*at\b|\b@)"
...
def _extract_entry_candidates(text: str) -> List[Tuple[str, List[float]]]:
    ...
    # Precio: @ / entry / buy at / sell at
    for m in re.finditer(r"(?:@|"+ENTRY_HINTS+r")\s*([+-]?\d[\d .,k]*)", norm, flags=re.IGNORECASE):
...
def _detect_action(text: str, direction: Optional[str]) -> Optional[str]:
    ...
    """
    low = text.lower()
    has_limit = re.search(LIMIT_WORD, low) is not None
    has_stop  = re.search(STOP_WORD, low)  is not None
    if direction == "BUY":
        if has_limit: return "BUY LIMIT"
        if has_stop:  return "BUY STOP"
        return "BUY"
    if direction == "SELL":
        if has_limit: return "SELL LIMIT"
        if has_stop:  return "SELL STOP"
        return "SELL"
    return None

# =========================
# Consistencia y score
# =========================

def _entrada_utilizable(entrada_obj: Dict[str, Any],
                        accion: Optional[str]) -> Tuple[Optional[float], Optional[str]]:
    """
    Devuelve (entrada_resuelta, entrada_fuente).
    - Si entrada.tipo == 'precio' -> (valor, "precio")
    - Si entrada.tipo == 'rango'  -> resuelve por accion:
        * BUY / BUY LIMIT / BUY STOP  -> min(rango)
        * SELL / SELL LIMIT / SELL STOP -> max(rango)
    - Otros casos -> (None, None)
    """
...
def _decidir_score(data: Dict[str, Any]) -> int:
    ...
    if es_valido and accion and (entrada_resuelta is not None) and sl_ok and tp_ok:
        return 10
    return 0
...
def _consolidar_entrada(cands: List[Tuple[str, List[float]]]) -> Dict[str, Any]:
    ...
    precios = [v for t, v in cands if t == "precio"]
    if rangos:
        if len(rangos) == 1:
    ...
    if precios:
        ...
        return {"tipo": "multiple", "valores": [p[0] for p in precios]}
    return {"tipo": "no_encontrada", "valores": []}
...
def clasificar_mensajes(texto: str) -> List[Dict[str, Any]]:
    ...
    for act in (activos or [None]):
        base = _build_output("Válido", act, accion, direccion,
                             entrada_obj, sl, tps, entrada_resuelta, entrada_fuente, consistencia, None)
        base["score"] = _decidir_score(base)
        salidas.append(base)
